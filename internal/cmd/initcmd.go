...
func (c *Config) builtinGitClone(repoURLStr string, workingTreeRawPath chezmoi.AbsPath) error {
	...
	for {
		...
		chezmoilog.InfoOrError(
			...
			isBare,
			"options",
			loggableGitCloneOptions(cloneOptions),
		)
...
func (c *Config) builtinGitInit(workingTreeRawPath chezmoi.AbsPath) error {
	...
	chezmoilog.InfoOrError(c.logger, "PlainInit", err, chezmoilog.Stringer("path", workingTreeRawPath), slog.Bool("isBare", isBare))
	return err
}

// LogValue implements golang.org/x/exp/slog.LogValuer.
func (o loggableGitCloneOptions) LogValue() slog.Value {
	var attrs []slog.Attr
	if o.URL != "" {
